...
1. edit `rust/crates/pi-ai/src/models.catalog.json`
2. run `cd rust && cargo run -p pi-ai --bin pi-ai-catalog -- fmt`
3. run `cd rust && cargo run -p pi-ai --bin pi-ai-catalog -- check`
4. keep provider filtering in `rust/crates/pi-ai/src/models.rs` aligned with the migrated provider set
5. update or extend `rust/crates/pi-ai/tests/models.rs`
6. use the TypeScript package as a compatibility reference, not as an input artifact
