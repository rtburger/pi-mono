...
async fn retries_429_retry_after_seconds() {
    ...
    let server = tokio::spawn(async move {
        ...
        assert!(first_line.starts_with("POST /chat/completions HTTP/1.1"));
        ...
        assert!(second_line.starts_with("POST /chat/completions HTTP/1.1"));
        write_http_response(
            &mut second_request,
            "200 OK",
            "text/event-stream",
...
async fn surfaces_last_error_after_max_attempts() {
    ...
    let server = tokio::spawn(async move {
        for body in ["fail-1", "fail-2", "fail-3"] {
            ...
            assert!(request_line.starts_with("POST /chat/completions HTTP/1.1"));
            write_http_response(
                &mut request,
                "503 Service Unavailable",
                "text/plain",
